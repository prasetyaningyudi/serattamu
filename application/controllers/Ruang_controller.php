...
class Ruang_controller extends CI_Controller {
	...
	public function index(){
		...
		$this->load->view('ruang_index_view', $data);
	...
	public function rekam(){
		if(isset($_POST['submit'])){			
			...
			$this->db->insert('RUANG_RAPAT', $data['saveddata']);
			header("location: " . base_url()."Ruang_controller/");
		}else{
			//Do nothing
		}		
		...
		$this->load->view('seg_head_view', $data);
		$this->load->view('seg_navigation_view', $data);
		$this->load->view('ruang_rekam_view', $data);
		$this->load->view('seg_footer_view', $data);		
	}
	...
	public function ubah($id=null){
		if($id == null){
			...
			if($data['record'] != null){
				...
				$this->load->view('seg_head_view', $data);
				$this->load->view('seg_navigation_view', $data);
				$this->load->view('ruang_ubah_view', $data);
				$this->load->view('seg_footer_view', $data);
				...
				if(isset($_POST['submit'])){
					...
					$this->db->update('RUANG_RAPAT', $data['saveddata']);	
					header("location: " . base_url()."Ruang_controller/");
				}else{
					//Do nothing
				}
